...
export default function FAQ() {
  ...
  const waUrl = data?.contact?.whatsappUrl || 'https://wa.me/6285641114777'
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-8">
        ...
        <div id="faqs-accordion-stack" className="space-y-4 text-left">
          {faqs.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-brand-bg rounded-2xl border border-brand-soft/50 overflow-hidden transition-all duration-300">
                <button
                ...
                >
                  <div className="flex gap-4 items-center pr-4">
                    ...
                    <span className="font-display font-bold text-sm sm:text-base text-brand-dark">{item.question}</span>
                  </div>
                  <div className={`w-8 h-8 rounded-full bg-white flex items-center justify-center text-brand-primary border border-brand-soft/30 shadow-xs transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}>
        ...
        <div className="mt-12 text-center bg-brand-soft/20 p-5 rounded-2xl border border-brand-soft/30 max-w-2xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a href={waUrl} target="_blank" rel="noopener noreferrer" className="btn-primary py-2 px-4 text-xs font-semibold rounded-full shrink-0">
            Hubungi HarmaFarm
          </a>
        </div>
      </div>
